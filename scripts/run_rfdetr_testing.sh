...
# Usage
#   Local (executable):
#     chmod +x run_rfdetr_testing.sh
#     ./run_rfdetr_testing.sh
#     ./run_rfdetr_testing.sh --weights output/checkpoint_best_total.pth --worst-overlap 10 --best-overlap 10
#
#   Slurm:
#     cd /path/to/Mosquitoes && sbatch run_rfdetr_testing.sh
...
if ! "${PYTHON_BIN}" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 10) else 1)'; then
  echo "ERROR: ${PYTHON_BIN} is too old. test_rfdetr_model.py requires Python >= 3.10." >&2
