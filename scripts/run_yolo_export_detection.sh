#!/usr/bin/env bash
#SBATCH --job-name=mosq-yolo-export-det
#SBATCH --time=00:30:00
#SBATCH --open-mode=truncate
#SBATCH --output=logs/yolo-detection-only-export-output.log
#SBATCH --error=logs/yolo-detection-only-export-error.log
#
# Detection-only, CPU-only: COCO (*_coco_det.json) + yolo_dataset_det/
# Logs: logs/yolo-detection-only-export-{output,error}.log
# Does NOT modify train_coco.json, yolo_dataset/, or multi-class runs/.
#
#   cd /path/to/mosquitoes-yolo && sbatch --export=ALL scripts/run_yolo_export_detection.sh
...
exec "${PYTHON_BIN}" "${SCRIPTS_DIR}/export_yolo_detection_dataset.py" "$@"
