#!/usr/bin/env bash
#SBATCH --job-name=mosq-yolo-convert
#SBATCH --time=00:30:00
#SBATCH --open-mode=truncate
#SBATCH --output=logs/yolo-end-to-end-convert-output.log
#SBATCH --error=logs/yolo-end-to-end-convert-error.log
#
# End-to-end (multi-class), CPU-only: writes train/val/test COCO JSON under <dataset>/labels/.
# Logs: logs/yolo-end-to-end-convert-{output,error}.log
# (No GPU required; omitting --gres avoids consuming a GPU for this step.)
#
#   cd /path/to/mosquitoes-yolo && sbatch run_yolo_conversion.sh
#   sbatch --export=ALL run_yolo_conversion.sh
#   sbatch run_yolo_conversion.sh --dataset /path/to/dataset_root
#
# Environment: MOSQUITOES_DATASET and/or KAGGLE_API_TOKEN — same rules as run_yolo_training.sh
...
See run_yolo_training.sh header for MOSQUITOES_DATASET, KAGGLE_API_TOKEN, .env.slurm, or --dataset.
EOF
  exit 1
fi
