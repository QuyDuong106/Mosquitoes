#!/usr/bin/env bash
#SBATCH --job-name=mosq-yolo
#SBATCH --time=06:00:00
#SBATCH --open-mode=truncate
#SBATCH --output=logs/yolo-end-to-end-train-output.log
#SBATCH --error=logs/yolo-end-to-end-train-error.log
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#
# End-to-end (multi-class): COCO split → yolo_dataset/ → train Ultralytics YOLO (see train_yolo_model.py).
# Logs: logs/yolo-end-to-end-train-{output,error}.log
#
# Default training behavior (passed explicitly below; extra args from sbatch go last and override):
#   --epochs 50 --patience 10 --model yolo11s.pt --imgsz 640
#   Example override: sbatch --export=ALL run_yolo_training.sh --model yolo11n.pt --epochs 80
#
# Submit from your repo clone (so logs and yolo_dataset/ land there):
#   cd /path/to/Mosquitoes && sbatch --export=ALL run_yolo_training.sh
#   sbatch --chdir=/path/to/Mosquitoes --export=ALL /path/to/Mosquitoes/run_yolo_training.sh
#   sbatch --export=ALL run_yolo_training.sh --dataset /scratch/you/mosquitoes-compsci760
#
...
#   sbatch --export=ALL run_yolo_training.sh
...
#     sbatch run_yolo_training.sh --dataset /path/to/dataset_root
...
if ! "${PYTHON_BIN}" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 10) else 1)'; then
  echo "ERROR: ${PYTHON_BIN} is too old. train_yolo_model.py requires Python >= 3.10." >&2
...
# Defaults align with train_yolo_model.py; append "$@" so sbatch CLI args override last-wins.
DEFAULT_TRAIN_ARGS=(
  --epochs 50
  --patience 10
  --model yolo11s.pt
...
exec "${PYTHON_BIN}" "${SCRIPTS_DIR}/train_yolo_model.py" "${DEFAULT_TRAIN_ARGS[@]}" "$@"
